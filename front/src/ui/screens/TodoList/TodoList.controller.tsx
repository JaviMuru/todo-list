...
export const TodoListController = () => {
  const {
    ...
    refetch,
    isLoading
  } = useQuery(['todos'], todoService.findAll);
  ...
  return (
    <TodoListView todos={todos} isLoading={isLoading} onSubmit={createTodo}
                  removeTodo={removeTodo} toggleTodoStatus={updateTodoStatus} />
  );
};
